...
// Structure so that we can return timing and maximum items read
class PerfInfo
{
	long time;
	long maxItems;	
...
public class SortTimer {
	...
	private static void doLoops( int sortAlg, String document, int numwords,
		...
		Sort12 sorter;
		PerfInfo perf = null; //allocated in doWork
		for (int i = 1; i <= steps; i++) {
            ...
            sorter = new HeapSort();
				...
				perf = doWork(sorter, document, numwords, doPrint);
				totalTime += perf.time; 
			}
	 ...
	 * @return PerfInfo (milliseconds,maxItems) taken to sort N words 
	 */
	private static PerfInfo doWork(Sort12 sorter, String document, int numwords,
			boolean doPrint ) {
		long start, end;
		...
		//ArrayList<String>  otherUnsorted = new ArrayList<String>(numwords);
